        ...
        async function fetchProducts() {
            try {
                ...
                if (res && res.ok) {
                    ...
                    const withPrices = shuffled.slice(0, 4).map(p => ({
                        ...p,
                        price: extractPrice(p)
                    }));
                    setSuggestedProducts(withPrices);
                } else {
                        ...
                        <div key={product.id} className="group cursor-pointer bg-white shadow-sm hover:shadow-md transition-shadow border border-gray-100 flex flex-col">
                            ...
                            <div className="p-5 text-center flex flex-col flex-1 justify-between">
                                <Link href={`/shop/product/${product.id}`} className="block">
                                    ...
                                    <p className="text-[#B38F52] font-semibold text-sm mb-4">₹{formatPrice(product.price)}</p>
                                </Link>
                                <button
                                    onClick={(e) => {
                                        e.preventDefault();
